...
- **Freeing disk space by emptying the Trash.** `~/.Trash` verified at 0
  bytes, but `df` still reported 43 GB free: 24 APFS local Time Machine
  snapshots still referenced the blocks, so the space was purgeable rather
  than free. Disk remained the binding constraint for 3D tooling.
